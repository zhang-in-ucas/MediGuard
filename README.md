...
- ✅ **可观测性**：每节点耗时统计、trace_id传播、结构化日志 —— 工业级基本功
...
```mermaid
flowchart TD
    A["用户输入"] --> B["分诊<br/>qwen-turbo"]
    B --> C["问诊生成<br/>qwen-plus<br/>+ RAG + 记忆"]
    C --> D["安全审查<br/>双层审查"]
    D -->|✅ 安全| E["格式化输出<br/>保存记忆"]
    E --> F([END])
    D -->|❌ 不安全| G{重试次数 < 3?}
    G -->|是，带回馈重试| C
    G -->|否，重试耗尽| H["兜底话术"]
...
```mermaid
flowchart TD
    Q["用户 Query"] --> V["向量检索<br/>ChromaDB<br/>text-embedding-v3"]
    Q --> B["BM25 关键词检索<br/>jieba 分词<br/>rank_bm25"]
    V --> V20["top-20"]
    B --> B20["top-20"]
